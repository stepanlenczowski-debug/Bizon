...
def reset_game():
    # Resetuj všechny globální proměnné na počáteční hodnoty
...
def spawn_bullet():
    # Spawnuj více objektů najednou (1-4)
...
def game_over_screen():
    # Smyčka pro obrazovku při konci hry
    while True:
        WIN.fill(BG)
        for text, y in [(FONT.render("GAME OVER", True, RED), HEIGHT//2 - 50),
...
def main():
    ...
    while running:
        ...
        # Pohyb hráče (4 směry)
        for key, dx, dy in [(pygame.K_a, -5, 0), (pygame.K_LEFT, -5, 0), (pygame.K_d, 5, 0), 
                            (pygame.K_RIGHT, 5, 0), (pygame.K_w, 0, -5), (pygame.K_UP, 0, -5), 
                            (pygame.K_s, 0, 5), (pygame.K_DOWN, 0, 5)]:
            if keys[key]:
                player.x += dx
                player.y += dy
                break
